...
@pytest.fixture(params=list(Path(DATA_DIR / "numpy" / "args").rglob("*.py"))[:5])
...
def testBaselineNewViolations(baselineFile, violationsFile: Path):
    violationsInAllFiles = _checkPaths((violationsFile,), exclude=EXCLUDE_PATTERN)
    generateBaseline(violationsInAllFiles, baselineFile)
    parsedBaseline = parseBaseline(baselineFile)
    ...
    newViolationsInAllFiles = _checkPaths((violationsFile,), exclude=EXCLUDE_PATTERN)
